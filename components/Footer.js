// handles the footer
 



export default function Footer() {
     return (
        <div className=" min-w-full items-center bg-[#E4E7EB] bg-opacity-95">
             <div className="grid grid-cols-4 sm:gap-28 md:gap-20 text-center m-0 py-10">

                <div className=" mt-5 mb-5 p-2 text-[#06202A]">
                    <p className="font-front-page-text-font tracking-widest text-3xl">PASADA</p>
                   
                </div>
                <div className=" text-[#06202A] mt-5 mb-5 p-2">
                <p className="text-lg font-front-page-text-font">Our Affiliates</p>
                    <p>first col</p>
                    <p>first col</p>
                </div>
                <div className="text-[#06202A] mt-5 mb-5 p-2">
                <p className="text-lg font-front-page-text-font">Schools</p>
                    <p>KNUST</p>
                    <p>Legon</p>
                </div>
                <div className=" text-[#06202A] mt-5 mb-5 p-2">
                <p className="text-lg font-front-page-text-font">Our Location</p>
                    <p>first col</p>
                    <p>first col</p>
                </div>
               


             </div>
         </div>
     )
 }
 